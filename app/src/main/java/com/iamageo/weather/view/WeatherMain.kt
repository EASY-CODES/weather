...
class WeatherMain : AppCompatActivity() {
    ...
    private fun setupViewWeatherInfo(response: WheaterResponse){
        ...
        binding.updateAt.text = "Updated at: "+ SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.ENGLISH).format(Date(response.dt * 1000))
        ...
        binding.sunrise.text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(response.sys.sunrise * 1000))
        binding.sunset.text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(response.sys.sunset * 1000))
